...
(async function(){
  ...
  try{
    const [A,F,FONT,jp,ws,nc,AN,F3]=await Promise.all([J('assets.json'),J('faces.json'),J('font.json'),J('jp.json'),J('ws.json'),J('nc.json'),J('anims.json').catch(()=>null),null]);
    window.ASSETS=A;window.ANIMS=AN;window.FACE3D=null;window.FACES=F;window.LOGO_FONT=FONT;window.COURSES={jp,ws,nc};
