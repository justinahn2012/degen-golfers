...
(async function(){
  ...
  try{
    const [A,F,FONT,jp,ws,nc,AN]=await Promise.all([J('assets.json'),J('faces.json'),J('font.json'),J('jp.json'),J('ws.json'),J('nc.json'),J('anims.json').catch(()=>null)]);
    window.ASSETS=A;window.ANIMS=AN;window.FACES=F;window.LOGO_FONT=FONT;window.COURSES={jp,ws,nc};
